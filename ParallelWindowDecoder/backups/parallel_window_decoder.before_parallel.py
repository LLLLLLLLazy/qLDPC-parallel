...
def decode_staggered_ab(
    ...
    for index, a_start in enumerate(a_starts):
        ...
        a_window_rows.append(s_range(row_start, row_stop))
        ...
        if oracle_errors is not None:
            total[:, commit_cols] = oracle_errors[:, commit_cols]
            continue
        ...
        for shot in range(shots):
            ...
            if ((mat @ local_e + det_data[shot, rows]) % 2).any():
                a_flagged += 1
            total[shot, commit_cols] = local_e[local_commit_start:local_commit_stop]
    ...
    for b_start in b_starts:
        ...
        rows = row_slice(problem, row_start, row_stop)
        cols = region_col_slice(problem, col_start, col_stop)
        b_window_rows.append(s_range(row_start, row_stop))
    ...
    diagnostics = {
        "schedule": "staggered",
        "a_windows": len(a_starts),
        "b_windows": len(b_starts),
        "a_window_rows": a_window_rows,
        "b_window_rows": b_window_rows,
        "a_source": "oracle" if oracle_errors is not None else "decoded",
        "a_noisy_boundary": a_noisy_boundary,
        "n_a": n_a,
        "n_a_solve": n_a_solve,
