...
cdef class osd_window:
    ...
    def __cinit__(self, parity_check_matrix, **kwargs):
        ...
        self.candidate_decodings = NULL
        self.candidate_weights = NULL
    ...
    cdef void clear_candidate_list(self):
        cdef int i, vn
        if self.candidate_list_size <= 0 or self.candidate_decodings == NULL:
            return
        for i in range(self.candidate_list_size):
            self.candidate_weights[i] = 1e308
            for vn in range(self.n):
                self.candidate_decodings[i][vn] = 0

    cdef void set_candidate_list_size(self, int top_k):
        cdef int i
        if self.candidate_decodings != NULL:
            for i in range(self.candidate_list_size):
                free(self.candidate_decodings[i])
            free(self.candidate_decodings)
            self.candidate_decodings = NULL
        if self.candidate_weights != NULL:
            free(self.candidate_weights)
            self.candidate_weights = NULL
        self.candidate_list_size = top_k
        if top_k <= 0:
            return
        self.candidate_decodings = <char**>calloc(top_k, sizeof(char*))
        self.candidate_weights = <double*>calloc(top_k, sizeof(double))
        for i in range(top_k):
            self.candidate_decodings[i] = <char*>calloc(self.n, sizeof(char))
        self.clear_candidate_list()

    cdef void push_candidate(self, char* decoding, double pm):
        cdef int insert_index = 0
        cdef int i, vn
        if self.candidate_list_size <= 0 or self.candidate_decodings == NULL:
            return
        while insert_index < self.candidate_list_size and self.candidate_weights[insert_index] <= pm:
            insert_index += 1
        if insert_index >= self.candidate_list_size:
            return
        for i in range(self.candidate_list_size - 1, insert_index, -1):
            self.candidate_weights[i] = self.candidate_weights[i - 1]
            for vn in range(self.n):
                self.candidate_decodings[i][vn] = self.candidate_decodings[i - 1][vn]
        self.candidate_weights[insert_index] = pm
        for vn in range(self.n):
            self.candidate_decodings[insert_index][vn] = decoding[vn]
    ...
    def decode_list(self, input_vector, int top_k=2, bint force_osd=False):
        cdef int input_length = input_vector.shape[0]
        cdef int vn, i, row_count
        cdef double pm
        cdef np.ndarray out
        ...
        if input_length != self.m:
            raise ValueError(f"The input to osd_window.decode_list must be a syndrome of length={self.m}. The inputted vector has length={input_length}.")

        self.synd = numpy2char(input_vector, self.synd)
        self.set_candidate_list_size(top_k)
        self.reset()
        self.bp_init()

        if self.bp_decode_llr(self.pre_max_iter):
            self.converge = True
            pm = 0.0
            for vn in range(self.n):
                if self.bp_decoding[vn]:
                    pm += self.channel_llr[vn]
            self.min_pm = pm
            if not force_osd or self.osd_order < 0:
                self.push_candidate(self.bp_decoding, pm)
                row_count = 1
                out = np.zeros((row_count, self.n), dtype=int)
                for vn in range(self.n):
                    out[0, vn] = self.bp_decoding[vn]
                return out
            self.osd()
        else:
            for vn in range(self.n):
            ...
            for vn in range(self.new_n, self.n):
                if self.vn_set_value(self.cols[vn], 0) == -1:
                    self.push_candidate(self.bp_decoding, 1e308)
                    row_count = 1
                    out = np.zeros((row_count, self.n), dtype=int)
                    for i in range(self.n):
                        out[0, i] = self.bp_decoding[i]
                    return out
            for vn in range(self.new_n, self.n):
                self.bp_decoding[self.cols[vn]] = 0
            if self.peel() == -1:
        ...
        row_count = 0
        for i in range(top_k):
            if self.candidate_weights[i] < 1e307:
                row_count += 1
        if row_count == 0:
            row_count = 1
            out = np.zeros((row_count, self.n), dtype=int)
            for vn in range(self.n):
                out[0, vn] = self.osdw_decoding[vn]
            return out

        out = np.zeros((row_count, self.n), dtype=int)
        for i in range(row_count):
            for vn in range(self.n):
                out[i, vn] = self.candidate_decodings[i][vn]
        return out
    ...
    cdef int osd(self):
        ...
        self.push_candidate(self.osd0_decoding, self.min_pm)
    ...
    def candidate_weights(self):
        cdef int i
        if self.candidate_list_size <= 0 or self.candidate_weights == NULL:
            return []
        return [
            self.candidate_weights[i]
            for i in range(self.candidate_list_size)
            if self.candidate_weights[i] < 1e307
        ]
